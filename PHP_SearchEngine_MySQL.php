...
//selecting table
mysqli_select_db($connections,"epiz_25762947_a") or die ( 'Unable to select database.' );
...
if(isset($_GET['search_term']) && isset($_GET['search_button']))
{
    ...
    $sql_query = mysqli_query($connections,"SELECT * FROM news WHERE MATCH(title,article) AGAINST('$search_term')");
    ...
    if($results = mysqli_num_rows($sql_query) != 0)
		...
		{
			$sql = "SELECT * FROM news WHERE (title LIKE '%".mysqli_real_escape_string($connections,$search_term)."%' OR article LIKE '%".$search_term."%') ";
			$sql_query = mysqli_query($connections,$sql);
			$results = mysqli_num_rows($sql_query);
			$sql_result_query = mysqli_query($connections,"SELECT * FROM news WHERE (title LIKE '%".$search_term."%' OR article LIKE '%".$search_term."%') LIMIT $first_pos, $RESULTS_LIMIT ");
		}
